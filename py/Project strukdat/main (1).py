...
def mainkan():
  ...
  while True:
    if str(user) not in ['1', '2', '3', '4', '5']:
      print("Invalid input, tolong pilih pilihan dengan benar!!")
      user = int(input("Masukkan pilihan(1,2,3,4,5): "))
      
    else:
      user = pilihan[user - 1]
      game = batuguntingkertas(user, com)
      game.opsi()
      game.permainan()
      playagain = input("Mau main lagi gak(y/n): ")
      if playagain.lower() == 'y':
        return mainkan()

      else:
  
        print("Terimakasih Sudah memainkan game kami")
        break
